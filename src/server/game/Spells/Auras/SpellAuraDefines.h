...
{
    SPELL_AURA_NONE                                         = 0,    // 无光环效果
    SPELL_AURA_BIND_SIGHT                                   = 1,    // 视线绑定，允许施法者通过目标单位的视角观察
    SPELL_AURA_MOD_POSSESS                                  = 2,    // 控制，允许施法者控制目标单位
    SPELL_AURA_PERIODIC_DAMAGE                              = 3,    // 定期造成伤害
    SPELL_AURA_DUMMY                                        = 4,    // 虚拟效果，通常用于特定的脚本逻辑
    SPELL_AURA_MOD_CONFUSE                                  = 5,    // 使目标混乱，失去控制方向
    SPELL_AURA_MOD_CHARM                                    = 6,    // 魅惑，使目标成为施法者的友方
    SPELL_AURA_MOD_FEAR                                     = 7,    // 使目标恐惧，通常使其逃跑
    SPELL_AURA_PERIODIC_HEAL                                = 8,    // 定期恢复生命值
    SPELL_AURA_MOD_ATTACKSPEED                              = 9,    // 改变攻击速度
    SPELL_AURA_MOD_THREAT                                   = 10,   // 修改仇恨值生成速率
    SPELL_AURA_MOD_TAUNT                                    = 11,   // 嘲讽，强制目标攻击施法者
    SPELL_AURA_MOD_STUN                                     = 12,   // 眩晕，使目标无法行动
    SPELL_AURA_MOD_DAMAGE_DONE                              = 13,   // 修改造成的伤害量
    SPELL_AURA_MOD_DAMAGE_TAKEN                             = 14,   // 修改承受的伤害量
    SPELL_AURA_DAMAGE_SHIELD                                = 15,   // 伤害护盾，受到攻击时反弹伤害
    SPELL_AURA_MOD_STEALTH                                  = 16,   // 潜行，使单位隐形
    SPELL_AURA_MOD_STEALTH_DETECT                           = 17,   // 增强潜行侦测，能够发现隐形单位
    SPELL_AURA_MOD_INVISIBILITY                             = 18,   // 使单位隐形，不同于潜行
    SPELL_AURA_MOD_INVISIBILITY_DETECT                      = 19,   // 增强隐形侦测，能够发现隐形单位
    SPELL_AURA_OBS_MOD_HEALTH                               = 20,   // 观察模式下修改生命值（非官方用途）
    SPELL_AURA_OBS_MOD_POWER                                = 21,   // 观察模式下修改能量值（非官方用途）
    SPELL_AURA_MOD_RESISTANCE                               = 22,   // 修改抗性
    SPELL_AURA_PERIODIC_TRIGGER_SPELL                       = 23,   // 定期触发另一个法术
    SPELL_AURA_PERIODIC_ENERGIZE                            = 24,   // 定期恢复能量（如法力、怒气等）
    SPELL_AURA_MOD_PACIFY                                   = 25,   // 平静，禁止攻击
    SPELL_AURA_MOD_ROOT                                     = 26,   // 定身，禁止移动但不阻止施法
    SPELL_AURA_MOD_SILENCE                                  = 27,   // 沉默，禁止施法
    SPELL_AURA_REFLECT_SPELLS                               = 28,   // 反射法术
    SPELL_AURA_MOD_STAT                                     = 29,   // 修改属性（如力量、智力等）
    SPELL_AURA_MOD_SKILL                                    = 30,   // 修改技能等级
    SPELL_AURA_MOD_INCREASE_SPEED                           = 31,   // 增加移动速度
    SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED                   = 32,   // 增加骑乘速度
    SPELL_AURA_MOD_DECREASE_SPEED                           = 33,   // 减少移动速度
    SPELL_AURA_MOD_INCREASE_HEALTH                          = 34,   // 增加生命值
    SPELL_AURA_MOD_INCREASE_ENERGY                          = 35,   // 增加能量（如法力、能量等）
    SPELL_AURA_MOD_SHAPESHIFT                               = 36,   // 变形或变换外观
    SPELL_AURA_EFFECT_IMMUNITY                              = 37,   // 免疫某种效果（不受某些特定效果影响）
    SPELL_AURA_STATE_IMMUNITY                               = 38,   // 免疫某种状态
    SPELL_AURA_SCHOOL_IMMUNITY                              = 39,   // 免疫特定的法术学派伤害
    SPELL_AURA_DAMAGE_IMMUNITY                              = 40,   // 完全免疫伤害
    SPELL_AURA_DISPEL_IMMUNITY                              = 41,   // 免疫被驱散
    SPELL_AURA_PROC_TRIGGER_SPELL                           = 42,   // 造成伤害时有几率触发另一个法术
    SPELL_AURA_PROC_TRIGGER_DAMAGE                          = 43,   // 受到伤害时有几率触发另一个法术
    SPELL_AURA_TRACK_CREATURES                              = 44,   // 跟踪生物类型
    SPELL_AURA_TRACK_RESOURCES                              = 45,   // 跟踪资源（如草药、矿石）
